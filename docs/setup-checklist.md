...
- [ ] Confirm success and error responses follow one documented JSON schema.
...
- [ ] Confirm all retry attempts preserve the original request ID for end-to-end correlation.
- [ ] Confirm the webhook response also exposes the request ID in a documented correlation header.
...
- [ ] Reconcile queued, sent, failed, and expired notification counts after a retry test.
...
- [ ] Confirm the final demo walkthrough also leaves the retry queue empty before capturing the closing portfolio screenshot.
